...
import java.util.regex.Pattern;
    ...
    public class UserRegistration {
        ...
        public static void main(String[] args) {
            ...
            System.out.println(email.validate("^[a-z\\d\\-]+(\\.[\\w+-]+)*@\\w+(\\.\\w+)?(?=(\\.[A-Za-z_]{2,3}$|\\.[a-zA-Z]{2,3}$)).*$", "swapnil-123@gmail.com.in"));
            // Phone number validation
            LambdaOperation phoneNumber = (regex, input) -> {
                return "Phone number validation is : " + Pattern.matches(regex, input);
            };
            System.out.println(phoneNumber.validate("[+]91 [6789]\\d{9}", "+91 9967891679"));
            //password validation
